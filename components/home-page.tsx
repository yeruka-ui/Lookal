...
// Define the product structure used in the randomized list
...
export function HomePage({ onNavigateToShop }: HomePageProps) {
    ...
    // FIX 1: Initialize the randomized list as empty array for SSR
    const [randomizedProducts, setRandomizedProducts] = useState<HomePageProduct[]>([])
    ...
    // FIX 2: Run randomization ONLY on the client after mount
    useEffect(() => {
        const allProducts = shops.flatMap((shop) =>
            shop.products.map((product) => ({ ...product, shopName: shop.name, shopId: shop.id }))
        )
        // Perform randomization client-side
        const shuffledProducts = allProducts.sort(() => Math.random() - 0.5).slice(0, 10)
    ...
    return (
        <div className="min-h-screen bg-background pb-24">
            ...
            {/* Fresh Arrivals (Masonry-ish) - Uses the fixed randomizedProducts state */}
            <section className="px-4 md:px-8 lg:px-32 py-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {/* FIX 3: Use randomizedProducts state */}
